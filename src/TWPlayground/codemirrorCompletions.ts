...
export function twCompletions(
  context: CompletionContext
): CompletionResult | null {
  const match = context.matchBefore(/[0-9a-z-_:\/\.]+/);
  if (!match) return null;

  const from = match.from;
  const prefix = match.text.toLowerCase();
  const filteredOptions = twClassNames
    .filter((item) => item.toLowerCase().includes(prefix))
    .map((item) => ({ label: item, type: "class" }));
